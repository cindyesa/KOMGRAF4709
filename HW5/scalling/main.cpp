...
void display() {
    ...
    // Matriks scaling
    glPushMatrix();
    glTranslatef(2.0, 2.0, 0.0);  // Pindahkan ke titik awal (2, 2)
    glScalef(scaleFactorX, scaleFactorY, 1.0);  // Scaling pada sumbu X
    glTranslatef(-2.0, -2.0, 0.0);  // Kembalikan ke titik awal (2, 2)

    // Gambar garis dari A(2,2) ke B(3,3)
    glBegin(GL_LINES);
    glVertex2f(2.0, 2.0);  // Titik A
    glVertex2f(3.0, 3.0);  // Titik B
    glEnd();
